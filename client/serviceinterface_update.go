...
func (cli *VanClient) ServiceInterfaceBind(ctx context.Context, service *types.ServiceInterface, targetType string, targetName string, protocol string, targetPorts map[int]int, namespace string) error {
	...
	if err == nil {
		...
		target, err := kube.GetServiceInterfaceTarget(targetType, targetName, deducePorts, svcNamespace, cli.KubeClient)
		if err != nil {
			return err
		}
...
func (cli *VanClient) GetHeadlessServiceConfiguration(targetName string, protocol string, address string, ports []int, publishNotReadyAddresses bool, namespace string) (*types.ServiceInterface, error) {
	svcNamespace := utils.GetOrDefault(namespace, cli.GetNamespace())
	statefulset, err := cli.KubeClient.AppsV1().StatefulSets(svcNamespace).Get(targetName, metav1.GetOptions{})
	if err == nil {
		...
		service, err := cli.KubeClient.CoreV1().Services(svcNamespace).Get(statefulset.Spec.ServiceName, metav1.GetOptions{})
		if err == nil {
